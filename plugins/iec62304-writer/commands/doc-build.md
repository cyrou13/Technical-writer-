...
3. **Release gate** (only with `--release`), in this order, stopping at
   ...
   d. **Exports** — for each exporter present in `tools/`
      ...
      `build_str_export.py`, `build_risk_export.py`, and the
      `build_risk_xlsx.py` / `build_use_export.py` exports): run it with
      `--release` (plus `--md-only` if the user asked for Markdown only).
      An exporter that does not know `--release` is the scaffolded
      working-draft generation (`tools/README.md`): it is **not** called
      with `--release`, and the summary says the release export for that
      deliverable needs the reference exporter synced.
